...
def process(cf):
    ...
    if cf.pred_model:
        model_predict = Model(input=model.input, output=model.get_layer('GAP').input)
        weights_LR = model.get_weights('dense')
        
        
        
        # Load images in a loop; forget about the generators for this.
        features = model.predict(img)
        
        # Class Activation Map for class 1:
        CAM1 = 
